...
Na inicializacao do runtime da Fase 5, todos os reles devem ser comandados para OFF antes de qualquer comando manual local. A excecao futura "quando um modo exigir restauracao" pertence as Fases 7 e 9 e nao deve ser implementada nesta fase.

O controle manual desta fase e local ao firmware. Ele deve permitir solicitar ON/OFF para cada rele e registrar `source` no System State usando os valores canonicos `LOCAL`, `MQTT`, `APP`, `AUTOMATION` e `FAILSAFE` ja definidos no estado global quando aplicavel ao contrato de entrada. Esta fase nao deve implementar transporte MQTT, comandos remotos, aplicativo, automacoes de ATO, modos operacionais, persistencia NVS, Alert Manager, cloud ou historico.
...
Criar os contratos minimos do modulo funcional de reles, incluindo identificadores canonicos dos quatro reles, comandos ON/OFF, origem da mudanca e interface substituivel de controle, permitindo testes automatizados sem hardware e sem acoplamento aos diagnosticos da Fase 1.
...
Definir uma interface funcional de controlador de reles que permita aplicar ON/OFF em um rele individual e aplicar OFF em todos os reles. A interface deve retornar resultado de sucesso, rele desconhecido, falha de GPIO ou contrato invalido, sem expor Arduino, `digitalWrite`, Serial real ou diagnosticos de bring-up.

Definir um contrato de comando manual local contendo rele alvo, estado desejado e origem da mudanca. A origem deve ser compativel com `RelaySource` do System State, mas esta tarefa nao deve implementar transporte remoto ou automacao.
...
- `firmware/include/modules/relays/relay_ids.h`
- `firmware/include/modules/relays/relay_controller.h`
- `firmware/include/modules/relays/relay_service.h`, somente se necessario para o contrato de comando
...
- A interface representa sucesso e falha de escrita.
...
- Existe contrato de comando manual local com rele alvo, estado desejado e origem da mudanca.
...
Implementar o controlador real dos reles em `drivers/relays/` ou area equivalente, separado do servico funcional. O controlador deve implementar a interface definida em `FW-P05-000` e usar `GpioPort` ou boundary equivalente para configurar GPIOs como saida e escrever niveis logicos.
...
A operacao `allOff` deve configurar todos os pinos de rele como saida quando necessario e escrever OFF em todos os reles, retornando falha se qualquer escrita obrigatoria falhar. A ordem deve ser deterministica e testavel.
...
- `firmware/src/drivers/io/gpio_interface.h`, somente se ajuste do boundary ja previsto for necessario
...
- Falhas de GPIO sao propagadas como resultado proprio do controlador funcional.
...
- Testar propagacao de falha simulada de GPIO.
...
Implementar mapeamento de estado e eventos locais de reles

### Objetivo

Criar a regra pura que transforma comandos de rele em alteracoes do bloco `relays`, preserva campos nao relacionados, registra `lastChanged` e `source`, e define os eventos locais `RELAY_ON` e `RELAY_OFF`.
...
Implementar um mapper de estado sem dependencias de Arduino ou hardware. O mapper deve receber o estado atual de `relays`, comando desejado, instante atual e origem da mudanca, retornando o novo `RelaysState` e o evento local esperado quando houver mudanca efetiva.
...
Comandos idempotentes, como ligar um rele ja ligado pela mesma origem sem mudanca relevante, nao devem gerar evento duplicado nem alterar `lastChanged` sem necessidade. Quando a origem mudar para o mesmo estado, o comportamento deve ser definido de forma deterministica e testado.
...
Esta tarefa nao deve escrever GPIO, nao deve atualizar o System State global, nao deve inicializar runtime e nao deve criar automacoes.

### Arquivos afetados

- `firmware/include/modules/relays/relay_state_mapper.h`
- `firmware/include/modules/relays/relay_events.h`
- `firmware/src/modules/relays/relay_state_mapper.cpp`
- `firmware/src/modules/relays/relay_events.cpp`
- `firmware/include/core/events/event_bus.h`
- `firmware/src/core/events/event_bus.cpp`, somente se necessario para novos tipos de evento
- `firmware/include/core/state/system_state.h`
- `firmware/src/core/state/system_state.cpp`, somente se ajuste de helper ja previsto for necessario
- `firmware/test/unit/`

### Criterios de aceitacao

- O mapper recebe comando de rele, estado atual, instante atual e origem da mudanca.
- Comando ON altera apenas o rele alvo para `enabled = true`.
- Comando OFF altera apenas o rele alvo para `enabled = false`.
- `lastChanged` do rele alvo e atualizado quando ha mudanca efetiva.
- `source` do rele alvo e atualizado com a origem informada quando ha mudanca efetiva.
- Campos dos demais reles sao preservados.
- Comando idempotente nao gera evento duplicado.
- O comportamento para mesma posicao com origem diferente e deterministico e coberto por teste.
- Existem contratos locais para `RELAY_ON` e `RELAY_OFF`.
...
- Executar build PlatformIO com sucesso.
- Testar comando ON para Recalque.
- Testar comando OFF para Recalque.
- Testar comando ON/OFF para Aquecedor.
- Testar comando ON/OFF para ATO.
- Testar comando ON/OFF para Reserva.
- Testar que apenas o rele alvo muda.
- Testar atualizacao de `lastChanged` em mudanca efetiva.
- Testar atualizacao de `source` em mudanca efetiva.
- Testar comando idempotente sem evento duplicado.
- Testar o comportamento definido para origem diferente com mesmo estado desejado.
- Testar que eventos `RELAY_ON` e `RELAY_OFF` sao escolhidos corretamente.
- Confirmar que os testes usam valores simulados e nao dependem de GPIO, ESP32 ou modulo de rele real.
...
Criar o servico funcional que aplica comandos manuais locais, aciona o controlador de rele, atualiza o bloco `relays` como fonte unica de verdade e emite eventos locais oficiais.
...
Implementar o servico do modulo de reles. Para cada comando manual local, o servico deve validar o rele alvo, solicitar a escrita ao controlador funcional, atualizar o System State somente apos sucesso da escrita e emitir `RELAY_ON` ou `RELAY_OFF` quando houver mudanca efetiva.

O servico deve usar `system.relays` como fonte unica de verdade para o estado exposto do modulo. Falha de escrita no controlador nao deve marcar o rele como ligado ou desligado no System State se o comando fisico nao foi aceito pelo boundary funcional.

O servico deve registrar `lastChanged` usando a fonte de tempo da Fase 2 e registrar `source` conforme a origem informada pelo comando. Nesta fase, a origem real usada pelo runtime deve ser `LOCAL` para comandos manuais locais e `FAILSAFE` para desligamentos de seguranca quando aplicavel. Valores `MQTT`, `APP` e `AUTOMATION` podem existir no contrato para compatibilidade do estado, mas seus transportes e automacoes nao devem ser implementados.

Implementar uma operacao funcional `allOff` para desligar os quatro reles, usada por boot seguro e por futuros fail-safes locais. A operacao deve atualizar o estado de cada rele efetivamente alterado e emitir eventos locais correspondentes sem duplicacao indevida.
...
- O servico chama o controlador funcional antes de atualizar o estado.
...
- Eventos de rele nao sao duplicados em comandos idempotentes.
- A operacao `allOff` desliga os quatro reles e atualiza o estado correspondente.
...
- Testar que comando idempotente nao duplica evento.
...
Durante `setup`, o runtime deve aplicar `allOff` antes de aceitar qualquer comando manual local. O estado inicial apos boot deve refletir os quatro reles desligados com `enabled = false`, usando origem deterministica. Restauracao de estado por modo ou persistencia nao deve ser implementada nesta fase.
...
A integracao nao deve registrar tarefa periodica desnecessaria para os reles se o modulo for puramente acionado por comando. Caso seja criado algum ponto de manutencao no scheduler, ele deve ser nao bloqueante, deterministico e justificado por necessidade real da implementacao.

O Watchdog deve continuar funcionando conforme a Fase 2. Falhas de inicializacao do controlador de reles devem ser reportadas de forma compatavel com o logger/core sem ligar reles indevidamente.
...
- O runtime aplica OFF nos quatro reles durante `setup`.
...
- Testar que a integracao nao registra tarefa periodica desnecessaria ou, se registrar, respeita comportamento deterministico.
...
- O runtime inicia os quatro reles desligados.
...
- Falhas de escrita no controlador nao corrompem o System State.
