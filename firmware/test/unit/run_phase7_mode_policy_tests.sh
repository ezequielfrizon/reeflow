...
BUILD_DIR="${TMPDIR:-/tmp}/reeflow_phase7_mode_policy_tests"
mkdir -p "$BUILD_DIR"
...
g++ -std=c++17 -Wall -Wextra -Werror \
  ...
  -I"$FIRMWARE_DIR/src" \
  -I"$FIRMWARE_DIR/test" \
  "$FIRMWARE_DIR/test/unit/test_mode_policy.cpp" \
  ...
  -o "$BUILD_DIR/test_mode_policy"

"$BUILD_DIR/test_mode_policy"
